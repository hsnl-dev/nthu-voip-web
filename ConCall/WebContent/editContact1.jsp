<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse, javax.servlet.http.HttpSession, talk.account.service.xsd.WebContact, talk.calling.service.xsd.NationalCode, util.Utility "%>
			...
			<div class="content-form">
            	...
            	<%
            		...
            		for(int i = 0; i < nation.length; i++){
            			if(contact.getMobilePhone().startsWith(nation[i].getNationalPreCode())){
            				precode = nation[i].getNationalPreCode();
            	%>
            		<option value="<%=nation[i].getNationalPreCode() %>" selected="selected"><%=nation[i].getNation() %></option>
            	<%
            			}else if(!contact.getMobilePhone().startsWith("+") & "台灣TAIWAN".equals(nation[i].getNation())){
            	%>
            		<option value="<%=nation[i].getNationalPreCode() %>" selected="selected"><%=nation[i].getNation() %></option>
            	<%
            			}else{
