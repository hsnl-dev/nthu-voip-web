...
<script type="text/javascript">
	function approve(phone){
		...
		alertify.confirm("確定要通過單位管理者帳號之申請嗎?", function (e) {
		    if (e) {
		        // user clicked "ok"
	...
	function reject(phone){
		alertify.set({ labels : { ok: "確定", cancel: "取消" } });
		alertify.confirm("確定要拒絕單位管理者帳號之申請嗎?", function (e) {
		    if (e) {
		        // user clicked "ok"
		    	document.getElementById("phone").value = phone;
					...
					List<User> webUserList = (List<User>)request.getAttribute("webUserList");
  					...
  					if(webUserList != null){
  						...
  						for(int i = 0; i < webUserList.size(); i++){
  							if(webUserList.get(i).isApproval() == false){
  								j++;
  					%>
  					<tr <%if(j%2 == 0){%>style="background-color:#e3e3e3;" <%} %>>
  						...
  						<td align="center">
							<a href="#" onclick="approve('<%=webUserList.get(i).getUsename() %>')" class="btn btn-primary">申請通過</a>
							<br>
							<a href="#" onclick="reject('<%=webUserList.get(i).getUsename() %>')" class="btn btn-default">申請拒絕</a>
							<br>
							<a href="#" onclick="deletewebuser('<%=webUserList.get(i).getUsename() %>')" class="btn btn-default">刪除管理者</a>
						</td>
  					</tr>
  					<%
  							}
