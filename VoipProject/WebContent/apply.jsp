<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="util.Unit,java.util.List,javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse, talk.account.service.xsd.UnitName "%>
...
function formsubmit(){
	...
	if(thisForm.unitId.value == ""){
		alertify.set({ labels : { ok: "確定", cancel: "取消" } });
		alertify.alert("請選擇單位名稱!");
		thisForm.unitId.focus();
		return false;
		...
		<div class="section1">
			<form id="applyForm" class="bs-docs-example form-horizontal" method="post" onsubmit="return check(this)" action="web.html">
            	...
            	<div class="input-group-lg apply-depart">
            		<select name="unitId" style="font-size:20px;">
            			<option value=""></option>
            	<% 
            		if(unitlist != null){
            			for(Unit one:unitlist ){
            	%>
        				<option value="<%=one.getUnitUuid() %>"><%=one.getUnitName()%></option>
        		<%
        				}
        			}
            	%>
