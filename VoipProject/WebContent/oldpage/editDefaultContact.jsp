<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse, javax.servlet.http.HttpSession, talk.account.service.xsd.WebContact "%>
...
function deleteDefaultContact(){
	var r=confirm("確定要刪除預設聯絡人嗎?");
	if (r==true){
		document.getElementById("action").value = "deleteDefaultContact";
		document.getElementById("contactForm").submit();
	}
}
</script>
        			...
        			<ul class="nav nav-pills">
				    	...
				    	%>
  						<li class="dropdown">
    						...
    						<ul class="dropdown-menu">
      							<li><a href="newDefaulContact.jsp">新增預設聯絡人</a></li>
...
<div id="content_column">
	<div class="content">
		...
		WebContact contact = (WebContact)session.getAttribute("editDefaultContact");
		%>
			<div class="sec1_alert">
				<div class="alert alert-success" width="200px">
					<b>
					您可在此修改或刪除<%=depart %>的預設聯絡人</b>
				</div>
			</div>
			<div class="sec1_text">
				<span class="label label-success"><h4>編輯預設聯絡人</h4></span>
				<form id="contactForm" class="bs-docs-example form-horizontal" method="post" onsubmit="return check(this)" action="default.do" enctype="multipart/form-data">
            		<div class="input-group input-group-lg">
              			聯絡人姓名:&nbsp;
                		<input type="text" class="form-control input-lg" id="cname" name="cname" value="<%=contact.getName() %>" placeholder="請輸入聯絡人姓名">
            		</div>
            		<div class="input-group input-group-lg">
              			行動電話號碼:&nbsp;
                		<input type="text" class="form-control input-lg" id="mobile" name="mobile" onkeyup="value=value.replace(/[^-_A-Z0-9]/g,'')" maxLength="10" value="<%=contact.getMobilePhone() %>" placeholder="請輸入聯絡人行動電話號碼">
            		</div>
            		<div class="input-group input-group-lg">
              			分機號碼:&nbsp;
                		<input class="span2" type="text" name="office" id="office" onkeyup="value=value.replace(/[^-_A-Z0-9]/g,'')" maxLength="8" value="<%=contact.getOfficePhone() %>" placeholder="請輸入930+分機號碼">
            		</div>
            		<div class="input-group input-group-lg">
            			<img src="img/<%=contact.getPic() %>" width="64px" height="85px"/>
              			上傳圖片:&nbsp;
                		<input type="file" name="pic"  value="瀏覽.." >
            		</div>
            		<div class="control-group">
                		<button class="btn btn-primary" type="submit" >確定</button>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="editDefault()" class="btn btn-default" type="button">取消</a>
                		&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" class="btn btn-default" onclick="deleteDefaultContact()">刪除</a>
           			</div>
           			<input type="hidden" name="action" id="action" value="updateDefaultContact"/>
          		</form>
			</div>		
		</div>
	</div>
