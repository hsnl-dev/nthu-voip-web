...
<script type="text/javascript">
	function approve(phone){
		var r=confirm("確定要通過管理者帳號之申請嗎?");
		if (r==true){
			//alert("管理者帳號已通過申請，系統將發信通知申請人。");
	...
	function reject(){
		var r=confirm("確定要拒絕管理者帳號之申請嗎?");
		if (r==true){
			//alert("已拒絕管理者帳號之申請，系統將發信通知申請人。");
			document.getElementById("phone").value = phone;
...
<body>
	...
	<div class="content">
		...
		<div class="section1">
			<h2><span class="label label-success">管理者帳號申請管理</span></h2>
				...
				WebAccount[] webUserList = (WebAccount[])request.getAttribute("webUserList");
  				...
  				if(webUserList != null && unitArray != null){
  					for(int i = 0; i < webUserList.length; i++){
  						if(webUserList[i].getApproval() == false){
  				%>
  					<tr>
  						...
  						<td align="center">
							<a href="#" onclick="approve('<%=webUserList[i].getUserName() %>')" class="btn btn-primary">申請通過</a>
							<br>
							<a href="#" onclick="reject('<%=webUserList[i].getUserName() %>')" class="btn btn-default">申請拒絕</a>
						</td>
  					</tr>
  				<%
  						}
