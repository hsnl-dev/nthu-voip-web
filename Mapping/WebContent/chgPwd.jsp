...
function formsubmit(){
	...
	if(mappingForm.oldPassword.value == ""){
		alertify.set({ labels : { ok: "確定", cancel: "取消" } });
		alertify.alert("請輸入舊的密碼!");
		mappingForm.oldPassword.focus();
		return false;
	}else if(old != mappingForm.oldPassword.value){
		alertify.set({ labels : { ok: "確定", cancel: "取消" } });
		alertify.alert("舊密碼錯誤，請重新輸入!");
		mappingForm.oldPassword.focus();
		return false;
	}else if(mappingForm.inputPassword.value == ""){
		alertify.set({ labels : { ok: "確定", cancel: "取消" } });
...
<!-- <img src="img/bg2.jpg" class="bg"> -->
	...
	<div class="content">
		<div class="section_text">
			...
			<% 
			if(status != null && "newUser".equals(status)){
			%>
			...
			%>
            	...
            	<div class="input-group-lg login-name">
            		<input type="password" class="form-control input-lg" id="oldPassword" name="oldPassword" placeholder="請輸入舊密碼">
            	</div>
            <% } %>
				<div class="content-text2">
					請輸入新密碼
            	...
            	<div class="input-group-lg login-name">
            		<input type="password" class="form-control input-lg" id="inputPassword" name="inputPassword" placeholder="請輸入新密碼">
            	</div>
            	<div class="content-text2">
					請再一次輸入新密碼
				</div>
            	<div class="input-group-lg login-name">
            		<input type="password" class="form-control input-lg" id="inputPassword2" name="inputPassword2" placeholder="請再一次輸入新密碼">
            	</div>
            	<div class="section-btn">
					<a href="#" onclick="formsubmit()"><img src="img/save_btn.png" height="70px" width="70px"></a>
				</div>
            </div>
            <input type="hidden" name="action" id="action" value="chgPwd">
			</form>
		</div>
	</div>
</body>
